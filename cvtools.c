...
float* FloatImageGaussianBlur(ClImage* src, Size ksize, double sigma1, double sigma2)
{
	int height, width, channel;
	int i, j, k, x, y;
	getClImageInfo(src, &height, &width, &channel);
	...
	float* dst = (float*)malloc(sizeof(float)*height*width*channel);
